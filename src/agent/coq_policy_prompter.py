...
class CoqGptPolicyPrompter(PolicyPrompter):
    ...
    def run_prompt(self, request: CoqGptResponse) -> list:
        prompt_messages = self.coq_gpt_response_grammar.format_as_per_grammar(request)
        prompt_message = self.agent_grammar.get_openai_main_message(prompt_message, "user")
        ...
        messages = self.system_messages + self._message_history + prompt_messages
